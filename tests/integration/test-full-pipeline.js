    ...
    try {
      ...
      for (const contact of data.contacts) {
        ...
        if (result && result.success) {
          ...
          console.log(`     ✅ Enriched with confidence: ${result.contact.confidenceScore || 'N/A'}`);
        } else {
          console.warn(`   Failed to enrich contact: ${contact.email} - ${result?.error}`);
        }

        console.log(`\n✅ Enriched ${enrichedContacts.length} contacts`);
        ...
        if (enrichedContacts.length > 0) {
          ...
          console.log(`     Company: ${sample.company?.name || 'N/A'}`);
          console.log(`     Confidence: ${sample.confidenceScore || 'N/A'}`);

          if (sample.intelligence) {
            console.log(`     Hooks: ${sample.intelligence.personalizationHooks?.length || 0}`);
          }
